...
export const askForElementLocationOrCreateNew = async (
  existingLocations: ReadonlyArray<string>,
  ...
  const choice = await showLocationsInQuickPick([
    ...
    ...existingLocations.map(toQuickPickItem),
  ]);
...
const askForLocationName = async (): Promise<string | undefined> => {
